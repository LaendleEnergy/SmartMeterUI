...
export default function DeleteDialog({ isOpen, setIsOpen, handleDelete, deleteUser = false, deleteDevice = false }: DeleteProps) {
  const subject = deleteUser ? 'User' : deleteDevice ? 'Gerät' : 'Account';
          ...
          <div className="min-h-screen px-4 text-center">
            ...
            >
              <div className="inline-block w-full max-w-md p-6 my-8 space-y-10 overflow-hidden text-center align-middle transition-all transform bg-indigo-50 shadow-xl rounded-lg justify-center items-center border-2 border-solid border-black">
                <Dialog.Title>
                  <span className="text-3xl font-bold pb-4">{subject} löschen</span>
                </Dialog.Title>

                <div className="flex-col space-y-6">
                  <div className="space-y-4">
                    <span className="text-sm sm:text-base pb-4">
                      Soll {deleteDevice ? 'dieses' : 'dieser'} {subject} wirklich endgültig gelöscht werden?
                    </span>
                    <br />
                  ...
                  <div className="inline-flex grow space-x-8 justify-center items-center">
                    <div className="bg-red-600 rounded-full p-3 transition duration-150 ease-in-out hover:bg-red-700 hover:shadow">
                      <button onClick={() => handleDelete(true)} className="text-sm sm:text-base text-center text-white font-medium leading-normal">
                        {subject} löschen
                      </button>
                    </div>
                    <div className="CancelButton bg-gray-400 rounded-full p-3 transition duration-150 ease-in-out hover:bg-gray-500 hover:shadow">
                      <button onClick={() => setIsOpen(false)} className="text-sm sm:text-basetext-center text-white font-medium leading-normal">
                        Abbrechen
                      </button>
                    </div>
                  </div>
