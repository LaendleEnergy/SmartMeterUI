...
export default function Rewards() {
  ...
  useEffect(() => {
    if (render) {
      fetch("http://localhost:8081/saving/getCurrentIncentive", {
        ...
        .then(async (res) => {
          if (res.ok) {
            ...
            var diff = Math.abs(
              formData.endDate.getTime() - new Date().getTime()
            );
      ...
      fetch("http://localhost:8081/leaderboard/get", {
        method: "GET",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          "Content-Type": "application/json",
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    const incentive: Incentive = {
      ...
      endDate: formData.endDate
        ? formData.endDate.toISOString().substring(0, 10)
        : "",
    };
    ...
    <div className="Rewards">
      ...
      <div className="flex flex-col justify-center items-center space-y-8 mb-5">
        ...
        <div className="flex flex-col space-y-5 bg-indigo-50 rounded-sm border-2 border-black p-5">
          ...
          <button
            className={
              editMode
                ? "hidden"
                : "Edit justify-center items-center inline-flex space-x-3 bg-primary-600 rounded-full p-2 transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow"
            }
            onClick={() => setEditMode(true)}
          >
            <div className="Bearbeiten text-sm sm:text-base text-white">
              Bearbeiten
            </div>
          ...
          <div
            className={
              editMode ? "hidden" : "text-base md:text-xl font-bold text-center"
            }
          >
            {formData.description}
          </div>
          <div
            className={editMode ? "hidden" : "text-sm md:text-lg text-right"}
          >
          ...
          <form
            ...
            onSubmit={submitForm}
            className={
              editMode ? "flex flex-col items-center space-y-3" : "hidden"
            }
          >
            ...
            <div className="flex grow space-x-4 md:space-x-8 mt-10 justify-center items-center">
              ...
              <div className="ConfirmButton bg-primary-600 rounded-full p-3 transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow">
                <button type="submit">
                  <span className="text-center text-white text-sm sm:text-base font-medium leading-normal">
        ...
        <div className="Leaderboard flex flex-col space-y-5 rounded-sm border-2 border-black bg-indigo-50 p-4">
          <div className="Rangliste text-lg md:text-xl font-bold">
            Rangliste
          </div>
          <div className="flex-col flex justify-center items-center space-y-8 mb-5">
            {leaderboardData.map((m, index) => (
              <DisplayAttribute
                key={m.id}
                name={`${m.name}: ${m.numberOfCreatedTags} Markierungen`}
                star={index == 0 ? true : false}
              ></DisplayAttribute>
            ))}
          </div>
        </div>
        <div className="ActiveButton inline-flex justify-center items-center bg-primary-600 rounded-full p-3 space-x-3 transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow">
          <Link href="../pages/energy-consumption">
            <span className="text-sm sm:text-base text-center text-white font-medium leading-normal">
              Jetzt Verbrauch zuordnen
            </span>
          </Link>
        </div>
      </div>
