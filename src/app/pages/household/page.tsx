...
export default function Household() {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (render) {
          const householdStatus: number = await fetchHouseholdData();

          if (householdStatus != 200) {
            householdStatus === 404 ? router.push('./errors/notfound') : router.push('./errors/error');
          }
    ...
    <div className="Household">
      ...
      <div className="flex-col flex justify-center items-center space-y-8 mb-5">
        ...
        <form method="POST" onSubmit={submitForm} className={editMode ? 'flex flex-col items-center space-y-2 md:space-y-4 border-2 bg-indigo-50 border-black border-solid p-2 md:p-5' : 'hidden'}>
          ...
          <div className="flex grow space-x-8 mt-10 justify-center items-center">
            <div className="CancelButton bg-gray-400 rounded-full p-3 transition duration-150 ease-in-out hover:bg-gray-500 hover:shadow">
              <button onClick={() => setEditMode(false)} className="text-center text-white font-medium text-sm sm:text-base">
                Abbrechen
              </button>
            </div>
            <div className="ConfirmButton bg-primary-600 rounded-full p-3 transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow">
              <button type="submit">
                <span className="Text text-center text-white font-medium text-sm sm:text-base">Änderungen übernehmen</span>
              </button>
            </div>
          </div>
        </form>
        <div className="ActiveButton inline-flex flex-col justify-center items-center bg-primary-600 rounded-full p-3 space-x-3 transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow">
          <Link href="../pages/members">
            <span className="Text text-center text-white font-medium text-sm sm:text-base">Haushaltsmitglieder verwalten</span>
          </Link>
        </div>
      </div>
      ...
      <div className="DeleteButton inline-flex fixed bottom-4 right-4 bg-red-600 rounded-full p-3 space-y-10 transition duration-150 ease-in-out hover:bg-red-700 hover:shadow">
        <button onClick={() => setIsOpen(true)} className="text-center text-white text-sm sm:text-base right grow w-50 h-15 inline-flex justify-center items-center space-x-3 ">
          Account löschen <FaTrash className="text-white"></FaTrash>{' '}
        </button>
      </div>
